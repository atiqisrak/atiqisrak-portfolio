...
export async function GET(request: NextRequest) {
  try {
    const externalApiUrl = 'https://www.news.aakashg.com/api/v1/homepage_data';
    ...
    const transformedData = {
      ...
      contentBlockData: {
        ...
        postsBySectionId: Object.fromEntries(
          Object.entries(data.contentBlockData?.postsBySectionId || {}).map(([key, posts]: [string, any]) => [
            key,
            posts.map((post: any) => transformPost(post))
          ])
        ),
        postsByTagId: Object.fromEntries(
          Object.entries(data.contentBlockData?.postsByTagId || {}).map(([key, posts]: [string, any]) => [
            key,
            posts.map((post: any) => transformPost(post))
          ])
        ),
      },
      newPosts: data.newPosts?.map((post: any) => transformPost(post)) || [],
      pinnedPosts: data.pinnedPosts?.map((post: any) => transformPost(post)) || [],
      topPosts: data.topPosts?.map((post: any) => transformPost(post)) || [],
    };
