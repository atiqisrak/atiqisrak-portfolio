...
import { ArrowLeft, ExternalLink, Calendar, User, Tag, Clock, Home, ChevronDown } from 'lucide-react';
import Link from 'next/link';
import { motion } from 'framer-motion';
import { Blog, BlogSection, BlogFAQ, BlogCTA } from '@/types/blog';
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
          ...
          <div className="flex items-center gap-2">
            ...
            <span>{Math.ceil(blog.metadata.word_count / 200)} min read</span>
          </div>
        </div>
      </div>
    </motion.header>
            ...
            <thead>
              <tr className="bg-muted">
                {table.headers.map((header, headerIndex) => (
                  <th key={headerIndex} className="border border-border p-3 text-left font-semibold">
                    {header}
                  </th>
                ))}
              </tr>
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <Button asChild variant="outline" size="sm">
              <Link href={author.profile_url} className="inline-flex items-center gap-2">
                View Profile
                <ExternalLink className="h-4 w-4" />
              </Link>
            </Button>
